...
import com.example.pos_backend.common.PosUtil;
...
public class MerchantController {
    ...
    public ResponseEntity<ApiResponse<Void>> registerMerchant(
        ...
        // 获取客户端信息
        String ipAddress = PosUtil.getClientIpAddress(request);
        String userAgent = request.getHeader("User-Agent");

        merchantService.registerMerchant(requestDTO, ipAddress, userAgent);

        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(null, "商家注册成功，已自动登录"));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> loginMerchant(
        ...
        merchantService.authenticateMerchant(email, password, ipAddress, userAgent);
        return ResponseEntity.ok(ApiResponse.success(null, "商家登录成功"));
    }
}
